...
const NestedFolder = () => {
  ...
  const [nestedFolderData, setNestedFolderData] = useState(null);
  ...
  useEffect(() => {
    if (id && folderId && isInitialLoad) {
      ...
      dispatch(fetchNestedFolders({
        ...
        setNestedFolderData(res?.payload?.data)
  ...
  const handleFolderClick = (folder) => {
    ...
    navigate(`${baseRoute}/detail/${id}/folder/${folderId}/nestedfolder/${folder.id}`);
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-none">
        <div className="flex items-center gap-2">
          <Text className="text-white font-bold">{nestedFolderData?.parentFolder?.name}</Text>
      ...
      <CreateRenameModal
        ...
        // Custom props for folder-specific text
        createTitle="Create Folder"
        renameTitle="Rename Folder"
        labelText="Folder Name"
        placeholder="Enter Folder Name"
        requiredMessage="Please enter a folder name"
        maxLengthMessage="folder name cannot exceed 25 characters"
      />

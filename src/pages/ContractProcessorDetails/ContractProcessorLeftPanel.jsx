import { Avatar, Button, Progress, Tag, Typography, message } from "antd";
...
const ContractProcessorLeftPanel = ({ userData, containerHeight }) => {
  ...
  // Determine base route based on current location
  ...
  // Check if current role is Loan Officer
  const isLoanOfficer = useMemo(() => {
    return location.pathname.includes("/loan-officer/");
  }, [location.pathname]);

  const isRealEstateAgent = useMemo(() => {
    return location.pathname.includes("/real-estate-agent/");
  }, [location.pathname]);
  ...
  const hasLikedFriendRequests =
    userData?.recievedFriendRequests?.is_like === true;
  ...
  const isBlockFunctionalityEnabled =
    friendRequest && friendRequest.id && Object.keys(friendRequest).length > 0;
  ...
  const handleHeartClick = () => {
    ...
    if (!friendRequest || !friendRequest.id) {
      message.error(
        "Unable to process request. Friend request data is missing."
      );
      return;
    }
    ...
    dispatch(
      ...
      .then(() => {
        ...
        dispatch(
          fetchContractProcessorUserDetails(userData?.id, {
  ...
  const handleBlockUnblockClick = () => {
    ...
    dispatch(
      ...
      .then((response) => {
        ...
        // If the action was to block the user, navigate to the dynamic base route
        if (newStatus === "block") {
          navigate(baseRoute);
        } else {
      ...
      >
        <div className="flex justify-center">
          {userData?.profile_photo_path ? (
            ...
            <Avatar
              ...
              className="w-28 h-28 object-cover rounded-full !text-5xl"
            // size={48}
            >
              {userData?.name?.[0]}
            </Avatar>
        ...
        <div className="mt-4 flex flex-col items-center">
          ...
          </Text>
          {
            ...
            <Text type="secondary" className="text-md break-all max-w-full overflow-wrap-anywhere mt-1 w-full text-left">
              {userData?.user_detail?.service_name}
            </Text>
          }
          ...
          {!isLoanOfficer && !isRealEstateAgent && (
            <div className="flex w-full gap-3 mt-1 ">
              ...
              {
                userData?.user_detail?.experience &&
                <div className="flex items-center">
                  <i className="icon-experience text-primary text-xl" />
                  <div className="text-sm">
                    {`${userData?.user_detail?.experience} Years`}
                  </div>
                </div>
              }
            </div>
          ...
          {!isLoanOfficer && !isRealEstateAgent && (
            ...
            <Text
              type="secondary"
              className="text-sm flex text-left text-justify leading-relaxed w-full"
              style={{
                lineHeight: '1.6',
                textAlign: 'justify',
            ...
            >
              {truncateText(userData?.user_detail?.description, 1100)}
            </Text>
          )}
          ...
          <div className="flex w-full gap-2 mt-2">
            <div className="flex items-center">
              ...
              <div className="text-xs break-all max-w-full overflow-wrap-anywhere">
                {userData?.email || ""}
              </div>
            </div>
          </div>
          ...
          <div className="flex w-full gap-2">
            ...
            <div className="flex items-center">
              ...
              <div className="text-xs">{userData?.phone_number || ""}</div>
            </div>
          </div>
        ...
        <div className="flex gap-3 flex-wrap 2xl:flex-nowrap 3xl:flex-nowrap mt-3">
          ...
          {isBlockFunctionalityEnabled && (
            <Button
              className={`py-4 flex-1 w-full text-sm ${isBlocked
                ? "bg-green-600 hover:bg-green-700 text-white"
                : "w-full bg-erroOpacityr border-error text-error text-sm sm:w-[calc(33.333%-0.5rem)]"
                }`}
          ...
          {isBlockFunctionalityEnabled && (
            <Button
              className={`px-1 ${hasLikedFriendRequests ? "text-red-500" : ""}`}
            ...
            >
              ...
              {!heartLoading && (
                <i
                  className={`${hasLikedFriendRequests
        ...
        <div className="mt-4 mb-3">
          ...
          <Tag className="bg-liteGray rounded-full text-gray-700 border-0 ml-1">
            ...
            {ratingData?.average_rating
              ? (Math.floor(ratingData.average_rating * 10) / 10).toFixed(1)
              : "0.0"}{" "}
          </Tag>
        </div>

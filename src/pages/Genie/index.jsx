...
import {
  ...
  fetchChatHistory,
} from "../../services/Store/Genie/action";
import moment from "moment";
import RenameChat from "../GenieChatDetails/Components/RenameChat";
import DeleteChat from "../GenieChatDetails/Components/DeleteChat";
import genieGIF from "../../assets/AI_Genie.gif";
import SelectCategory from "../GenieChatDetails/Components/SelectCategory";
...
const Genie = () => {
  ...
  const fetchHistory = (pageNum = 1, searchTerm = "") => {
    ...
    dispatch(fetchChatHistory(params))
      .then((res) => {
  ...
  const handleSend = () => {
    if (userQuestion.trim()) {
      ...
      dispatch(createRecommendationAction(payload))
        .then((res) => {
          console.log("new chat res", res);
          navigate(
            `/ai-genie/${res?.payload.data.conversation.conversation_id}`
          );
  ...
  const commonQuestions = [
    {
      id: 1,
      question: "What lenders offer FHA new construction loans?",
    },
    {
      id: 2,
      question: "What lenders offer bridge loans in California?",
    },
    {
      id: 3,
      question: "Who do you recommend for construction-to-permanent loans?",
    },
    {
      id: 4,
      question: "Need an ITIN lender for a purchase transaction with maximum CLTV of 95%.",
    },
  ];
      ...
      <div className="">
        <div className="flex flex-wrap gap-2">
          {hiddenCategories.map((category) =>
            renderCategoryTag(category, true)
          )}
        </div>
      </div>
    );
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          {hiddenCategories.length > 0 && (
            ...
            >
              <Tag className="flex items-center px-3 py-1 rounded-full border-none text-white bg-gray-600 hover:bg-gray-500 cursor-pointer whitespace-nowrap">
                <EllipsisOutlined
      ...
      >
        <div className="flex justify-center items-center">
          ...
          >
            ...
            <Text className="text-white text-lg sm:text-2xl">AI Genie</Text>
          </Title>
        </div>
      </Col>
          ...
          <div className="rounded-3xl border overflow-hidden bg-gray border-solid border-liteGray w-full relative">
            ...
            >
              <Row className=" h-full" gutter={[24, 24]}>
                ...
                >
                  <i
                    className="block lg:hidden icon-down-arrow rotate-90 items-center justify-center absolute z-50 top-0 left-[-4%] md:left-[-1%]"
                    style={{ fontSize: "24px" }}
                    onClick={onClickBack}
                  />
                  <div className="w-[95%] lg:w-[75%] my-5 lg:my-0 ">
                    <Row justify="center">
                      ...
                      >
                        ...
                        <Text
                          className="text-white text-4xl font-bold mt-2 mb-4 pb-2"
                          style={{
                            background:
                        ...
                        <Text className="block mx-auto max-w-4xl">
                          {`Let's find the best loan options for your client using 10,000+ pages of our approved lenders guidelines.
                          Ask about requirements, eligibility, or plans for instant expert insights.`}
                        </Text>
                      </Col>
                    </Row>

                    <Row justify="center" className="mb-8 mt-8">
                      <Col xs={24}>
                          ...
                          <Form form={form} requiredMark={false}>
                            <Form.Item name="question" className="mb-0">
                              <Input
                                ...
                                placeholder={innerWidth < 640 ? "Message..." : "Send a message..."}
                                size="large"
                                className={`rounded-full bg-darkGray w-full ${innerWidth < 640 ? 'pr-[10rem]' : 'pr-[16.5rem]'} py-4 h-[68px] border-liteGray`}
                              />
                            </Form.Item>
                          </Form>
                          ...
                          {recommendationLoading ? (
                            ...
                            <Button
                              ...
                              disabled={
                                ...
                                recommendationLoading || selectedCategories.length <= 0
                              }
                            >
                              Send
                            </Button>
                    ...
                    <Row gutter={[16, 16]} className="mt-4">
                      {commonQuestions.map((item) => (
                        <Col xs={24} md={12} key={item.id}>
                          ...
                          >
                            <Text className="text-white text-base !text-left break-words text-wrap w-full">
                              {item.question}
                            </Text>
                          </Button>
                        </Col>

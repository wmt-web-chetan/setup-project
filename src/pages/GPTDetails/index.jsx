import { Button, Col, Form, Input, Row, Typography, Dropdown, Modal, Tooltip, Skeleton } from 'antd';
import React, { useEffect, useState, useRef } from 'react'
import './GPTDetails.scss'
...
import { createRecommendationActionGPT, fetchChatConversationsGPT, fetchChatHistoryGPT } from '../../services/Store/GPT/action';
...
const GPTDetails = () => {
    ...
    const { chatHistory, chatHistoryLoading, recommendationLoading, renamedConversation, deletedConversation } = useSelector((state) => state?.gpt);
    ...
    const fetchHistory = (pageNum = 1, searchTerm = '') => {
        ...
        dispatch(fetchChatHistoryGPT(params)).then((res) => {
    ...
    const fetchConversationMessages = (pageNum = 1, loadMore = false) => {
        ...
        dispatch(fetchChatConversationsGPT(payload)).then((res) => {
    ...
    const handleSend = () => {
        if (userQuestion.trim()) {
            ...
            dispatch(createRecommendationActionGPT(payload)).then((res) => {
                console.log('new chat res', res?.payload?.data);

                // Check if there's a response message to add
                const responseMessages = res?.payload?.data?.conversation?.messages || [];
                const assistantMessage = responseMessages.find(msg => msg.isResponse === true);

                if (assistantMessage) {
                    // Add only the assistant's response to the conversation
                    setConversationData(prevData => ({
                        ...prevData,
                        conversation: {
                            ...prevData.conversation,
                            conversation_id: res?.payload?.data?.conversation?.conversation_id || prevData.conversation.conversation_id,
                            messages: [...prevData.conversation.messages, assistantMessage]
                        }
                    }));
                }
    ...
    return (
        <Row
            ...
            gutter={[0, 24]}
        >
            <GeminieVoiceSearchModal openVoiceModal={openVoiceModal} setOpenVoiceModal={setOpenVoiceModal} from='detail' handleSend={handleSend} setUserQuestion={setUserQuestion}/>
                                        ...
                                        <div className=" h-full">
                                            ...
                                            >
                                                ...
                                                {conversationData?.conversation?.messages?.map((item, index) => {
                                                    return (
                                                        item?.isResponse ?
                                                            <div className='flex' key={`message-${index}`}>
                                                                <div className='pr-3'>
                                                                    <Button
                                                                        type="text"
                                                                ...
                                                                /> */}
                                                                <HtmlMessage content={item?.content} from={'GPT'}/>
                                                            </div>
                                                            :
                                            ...
                                            <div className='chatInput' >
                                                <div className="relative w-full"
                                                    style={{ boxShadow: "#1e1e1ea4 11px -20px 20px 20px" }}
                                                >
                                                    ...
                                                    <Button
                                                        ...
                                                        disabled={userQuestion?.length < 1 || recommendationLoading}
                                                    >
                                                        Send
                                                    </Button>
                                                </div>
